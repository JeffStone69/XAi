#!/usr/bin/env python3
"""
🌍 GeoSupply Short-Term Profit Predictor v2.3
Major upgrades: robust Tickers fetch, RSI + z-score normalized signals,
sector filter, stronger Grok prompt, cleaner code.
"""
...
st.set_page_config(page_title="GeoSupply v2.3", page_icon="🌍", layout="wide")

# ====================== CONFIG ======================
SECTORS = {
    "Mining": ["BHP.AX","RIO.AX","FMG.AX","FCX","NEM","VALE"],
    "Shipping": ["ZIM","MATX","SBLK","QUB.AX"],
    "Energy": ["XOM","CVX","STO.AX","WHC.AX","CCJ"],
    "Renewable": ["NEE","FSLR","ENPH","IGO.AX"],
    "Tech": ["NVDA","TSLA","WTC.AX","XRO.AX"]
}
ALL_TICKERS = list(dict.fromkeys(sum(SECTORS.values(), [])))

logging.basicConfig(filename="geosupply.log", level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
...
def compute_signals(raw_data, horizon="5d", selected_sectors=None):
    ...
    for ticker, df in raw_data.items():
        ...
        try:
            ...
            vol_spike = df["Volume"].iloc[-1] / df["Volume"].mean()
            rsi = 100 - (100 / (1 + ret[ret>0].mean() / -ret[ret<0].mean())) if len(ret)>0 else 50
            vol = ret.std() * 100

            score = round(price_change * min(vol_spike, 3) * (rsi/50) / (1 + vol/15), 2)
    ...
    if not df.empty:
        df["Signal"] = round((df["Signal"] - df["Signal"].mean()) / df["Signal"].std(), 3)  # z-score
        df = df.sort_values("Signal", ascending=False).reset_index(drop=True)
    return df

# ====================== GROK ======================
def call_grok(prompt, model):
    if not st.session_state.get("api_key"): return "Enter Grok API key."
    try:
        r = requests.post(
...
st.title("GeoSupply Short-Term Profit Predictor **v2.3**")
st.caption("Robust • RSI + Normalized Signals • Real Edge in Geo-Supply Chains")
...
with tab1:
    st.subheader(f"Leaderboard — {horizon} | Updated {datetime.now().strftime('%H:%M')}")
    if not df.empty:
        st.dataframe(df.style.background_gradient(subset=["Signal"], cmap="RdYlGn"), use_container_width=True, hide_index=True)
        st.download_button("Export CSV", df.to_csv(index=False), f"geosupply_{horizon}.csv")
    else:
        st.error("No data available.")

with tab2:
    st.subheader("Top 5 Charts")
    for ticker in df.head(5)["Ticker"].values:
...
with tab3:
    ...
    if st.button("Generate Thesis", type="primary"):
        with st.spinner("Calling Grok..."):
            prompt = f"""Analyze this geo-supply leaderboard for 2-5 day trading edge.
Current top signals:\n{df.head(12).to_string(index=False)}
Focus on supply chain tension, momentum conviction, and catalyst potential.
Return exactly 3 high-signal bullets."""
            response = call_grok(prompt, model)
            st.markdown(response)

st.caption("v2.3 • Improved signals • Robust fetch • Better AI thesis")
